...
export default {
    unauthorizedSave: (m: Model) => new Error(`You've attempted to call save in the ${m.isCollection() ? 'Collection' : 'Model'} ${m.constructor.name} but you either didn't specify it as a connected ${m.isCollection() ? 'Collection' : 'Model'} or didn't specify the config has done at the root of your project: "config.done()"`),
    unauthorizedCookieAdd: (m: Model) => new Error(`You've attempted to call cookie in the ${m.isCollection() ? 'Collection' : 'Model'} ${m.constructor.name} (key: ${m.options.key}), but this functionnality is unavailable in it for these reasons:\n1. Doesn't have a unique specified key in the building options.\n2. It is not connected to the store.\n3. You didn't specify the config has done at the root of your project: "config.done()"\n4. You are using React Native`),
    unauthorizedLocalStore: (m: Model) => new Error(`You've attempted to call localStore in the ${m.isCollection() ? 'Collection' : 'Model'} ${m.constructor.name} (key: ${m.options.key}), but this functionnality is unavailable in it for these reasons:\n1. Doesn't have a unique specified key in the building options.\n2. It is not connected to the store\n3. You didn't specify the config has done at the root of your project: "config.done()"\n4. You are using NextJS.`),
